...
# if player and enemy collides, end game (for example running = false))
...
class Game:
    def __init__(self):
        screen_width = 1200
        screen_height = 1000
        self.player = Player(0, 700)
        self.enemy = Player(200, 700)
        self.enemy_real = Enemy(100, 700)
    ...
    def run(self):
        ...
        while running:
            ...
            if keys[pygame.K_LEFT] and self.canvas.check_if_inside_left_boundary(self.player):
                self.player.move_left()

            elif keys[pygame.K_RIGHT] and self.canvas.check_if_inside_right_boundary(self.player):
